...
{
	*resultado = 0;
	if(letra == '(' || letra == ')' || letra >= 'A' || letra <= 'Z' || letra >= '0' || letra <= '9' || letra == '+' || letra == '.')
		return 1;
...
{	
	...
	for(i = 0; i < strlen(expressao); i++)
	{
		...
		{						
			int find = indexOf(expressao, '(');
			while(find > -1)
			{
				int last = lastIndexOf(expressao, ')');
				char* sub = substr(expressao, find, last);

				printf("%s\n", sub);
				if(!carrega(interpretador, sub))
				{
					return 0;
				}
				else
				{
					expressao = removeStr(expressao, find - 1, last);
					expressao = insertStr(expressao, find, interpretador->saida ? "1" : "0");					
				}
			}
		}
	}
